...
static const Header good_header = {
    ...
    { 0x24, 0xFF, 0xAE, 0x51, 0x69, 0x9A, 0xA2, 0x21, 0x3D, 0x84, 0x82, 0x0A, 0x84, 0xE4, 0x09, 0xAD,
        ...
        0xD6, 0x25, 0xE4, 0x8B, 0x38, 0x0A, 0xAC, 0x72, 0x21, 0xD4, 0xF8, 0x07 }, /* title */
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, /* game code */
    0x00000000, /* maker code */
    0x3130, /* fixed */
    0x96, /* unit_code */
    0x00, /* device type */
    0x80, /* unused */
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, /* game version */
    0x00, /* complement */
    0x00, /* checksum */
    0x0000
};
...
static void applyheader(Header* header, const Header* addheader, int istitle, int isgamecode, int ismakercode, int isversion, int isdebug)
{
    ...
    if (istitle)
        memcpy(header->title, addheader->title, sizeof(header->title));

    if (isgamecode)
        header->game_code = addheader->game_code;

    if (ismakercode)
        header->maker_code = addheader->maker_code;

    if (isversion)
        memcpy(&header->game_version, &addheader->game_version, sizeof(header->game_version));
